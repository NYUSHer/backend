...
class PostTestCase(BasicTestCase):
    ...
    def test_get(self):
        ...
        data = json.loads(rv.get_data().decode('utf-8'))
        assert data['title'] == 'test3'
        assert data['tags'] == 'sad'
        assert data['authorid'] == 1

    def submit_new(self, user_id, token, title, category, tags, content):
    ...
    def test_submit_new(self):
        rv = self.submit_new(1, 'd33341c4-e55c-4e65-8a0b-af39a86a2fce', "test new", "cafeteria", "sad", "There was a bug in my salad....")
        data = json.loads(rv.get_data().decode('utf-8'))
    ...
    def test_submit_modify(self):
        rv = self.submit_modify(1, 'd33341c4-e55c-4e65-8a0b-af39a86a2fce', "test modify", "cafeteria", "happy", "how are you", 2)
        data = json.loads(rv.get_data().decode('utf-8'))
        assert data['pid'] is not None
